...
public class TKLoaiPhongGUI extends JPanel {
 //setbounds(x,y,width,length)
    ...
    JButton timkiem, thunho, exit,in;
    ...
    public TKLoaiPhongGUI() {
        ...
        lbtitle = new JLabel("Thống kê theo loại phòng");
        ...
        timkiem.addMouseListener(new MouseListener() 
        {
            ...
            ThongKeLoaiPhongBUS bustk = new ThongKeLoaiPhongBUS();
//        if (ThongKeLoaiPhongBUS.dstkptheongay == null) {
            bustk.docDSPngay(String.valueOf(txngaybd.getText()), String.valueOf(txngaykt.getText()));
  //      }
        
        for (ThongKeLoaiPhongDTO tkptn : ThongKeLoaiPhongBUS.dstkptheongay) {
            Vector row = new Vector();
            row.add(tkptn.getLoaiphong());
            row.add(tkptn.getSoluotdat());
            model.addRow(row); 
            table.setModel(model);
        }
            }
    ...
    public void setEvent(){
        ThongKeLoaiPhongBUS bustkdv = new ThongKeLoaiPhongBUS();
        if (ThongKeLoaiPhongBUS.dstkp == null) {
            bustkdv.docDSP();
        }
        
        for (ThongKeLoaiPhongDTO tkpdv : ThongKeLoaiPhongBUS.dstkp) {
            Vector row = new Vector();
            row.add(tkpdv.getLoaiphong());
      ...
      public static void main(String[] args) {
        TKLoaiPhongGUI a=new TKLoaiPhongGUI();
            //a.setVisible(true);
            JFrame f=new JFrame();
            f.setSize(1500,800);
